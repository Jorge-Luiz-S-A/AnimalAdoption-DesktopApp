...
class SearchTab(BaseTab):
    def __init__(self, parent):
        ...
        self.e_species = ttk.Entry(filt_row1, width=18); self.e_species.grid(row=0, column=1, padx=(0, 15))
        
        ttk.Label(filt_row1, text="Porte").grid(row=0, column=2, padx=(0, 5))
        self.cb_size = ttk.Combobox(filt_row1, values=SIZES, state="readonly", width=14); self.cb_size.grid(row=0, column=3, padx=(0, 15))
        
        ttk.Label(filt_row1, text="Local").grid(row=0, column=4, padx=(0, 5))
        self.e_location = ttk.Entry(filt_row1, width=18); self.e_location.grid(row=0, column=5)
        
        # Segunda linha de filtros
        filt_row2 = ttk.Frame(filt)
        filt_row2.pack(fill=tk.X, padx=10, pady=(0, 10))
        
        ttk.Label(filt_row2, text="Idade min").grid(row=0, column=0, padx=(0, 5))
        self.e_amin = ttk.Entry(filt_row2, width=8); self.e_amin.grid(row=0, column=1, padx=(0, 15))
        
        ttk.Label(filt_row2, text="Idade max").grid(row=0, column=2, padx=(0, 5))
        self.e_amax = ttk.Entry(filt_row2, width=8); self.e_amax.grid(row=0, column=3, padx=(0, 15))
    ...
    def search(self):
        ...
        species = self.e_species.get().strip().lower()
        size = self.cb_size.get().strip().lower()
        location = self.e_location.get().strip().lower()
        ...
        results = q.all()
        for a in results:
            self.tree.insert("", "end", values=(a.id, a.name, a.species, a.age, a.size or "", a.gender or "", a.status, a.location or ""))
